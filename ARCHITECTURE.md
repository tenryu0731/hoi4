...
| アセット | 数 | 形式 | 生成方法 |
|---|---|---|---|
| 国旗 | 40 | SVG (60×40) | 史実 1936 年の旗を矩形 / 十字 / 三色 / 紋章プリミティブの組合せで記述したテーブルから生成 |
| NATO 兵科記号 | 12 | SVG (48×32) | 歩兵 (×)、機甲 (楕円)、自動車化、砲兵 (●)、山岳 (∧)、海兵、空挺、騎兵 等の標準記号 |
| 資源アイコン | 6 | SVG (32×32) | 石油・鋼鉄・アルミ・タングステン・ゴム・クロム のシルエット |
| UI アイコン | 14 | SVG (32×32) | 工場、造船所、人的資源、政治力、研究、外交、生産、建設、再生、ポーズ 等 |
| 地形テクスチャ | 1 | 手続き生成 → `RenderTexture` | value noise + fBm で起伏の陰影を実行時ベイク (帯域ゼロ) |
| フォント | 1 | Bitmap font (Pixi 生成) | システムフォントから `BitmapFont.install` で実行時生成 |

`public/assets/manifest.json` に全アセットのパス・サイズを記録し、テストで合計容量と読み込み時間を検証する
(**閾値: 全アセット合計 ≤ 300KB、初回ロード ≤ 1500ms**)。
...
```
npm run test:e2e
  ├ screenshot   Playwright で 6 シーンを撮影 → 前回とピクセル差分
  │              変更のあった矩形領域に絞って比較 (差分率 > 0.5% で fail)
  ├ perf         iPhone 12 相当 (390×844, DPR 3 → 実効 2) で 600 フレーム計測
  │              閾値: p50 ≤ 16.6ms, p95 ≤ 24ms, p99 ≤ 33ms  (= 30fps 以上)
  ├ touch        パン / ピンチ / タップ選択 / 部隊ドラッグ移動 のスクリプト検証
  └ assets       manifest の合計容量 ≤ 300KB、初回ロード ≤ 1500ms
```
